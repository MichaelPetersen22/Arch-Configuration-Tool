...
#         prompt = True
#         while(prompt):
#             path = input("Please Input Package List Path: ")
#             print(
#                 "\033[A                                                                        \033[A")
#             if exists(path):
#                 ops = ['Yes', 'No']
#                 menu = TerminalMenu(
#                     ops, title="Do you want to specify additional packages to be enabled on startup?")
#                 selection = menu.show()
#                 if (ops[selection] == 'Yes'):
#                     specifyEnabled = True
#                 else:
#                     specifyEnabled = False
#                 return [path, True, specifyEnabled]
#             elif not path:
#                 return ['', False, False]
#             else:
...
#     def packages(UI):
#         enableprompt = UI.packageList[2]
    # def packageEnable(UI):
    #     enablePrompt = UI.packagelist[2]
    #     if enablePrompt:
    #         menuopts = []
    #         preselected = []
    #     with open(UI.packagelist[1]) as applist:
    #         index = 0
    #         installmode = 'pacman'
    #         for line in applist:
    #             if ('AUR' in line):
    ...
    #             UI.packages.append([split[0], installmode, enabled])
    #             if enablePrompt:
    #                 menuopts.append(split[0])
    #             index += 1
    #     if enablePrompt:
    #         menu = TerminalMenu(menuopts, title="Select Packages to autostart", multi_select=True,
    #                             preselected_entries=preselected, multi_select_select_on_accept=False, multi_select_empty_ok=True)
    #         selection = menu.show()
        ...
        while(prompt):
            ...
            if exists(path):
                with open(path) as jsonfile:
                    ...
                    UI.aur = data['aur']
                    UI.packagelist = data['packagelist']
